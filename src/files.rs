...
/// Find the first nvim Unix socket.
/// On Linux: /run/user/<uid>/nvim.*.0
/// On macOS:  $TMPDIR/nvim.*/0  (or nvim.*.0 — same pattern, different dir)
#[cfg_attr(target_os = "macos", allow(dead_code))]
fn find_nvim_socket() -> Option<PathBuf> {
    ...
    for entry in rd.flatten() {
        let name = entry.file_name().to_string_lossy().to_string();
        // macOS: nvim.<pid>/ directory containing socket "0"
        if name.starts_with("nvim.") && entry.path().is_dir() {
            let sock = entry.path().join("0");
            if sock.exists() {
                return Some(sock);
            }
        }
        // Linux / fallback: nvim.<pid>.0 file
